...
Editor = function(chromeless, themes, model, graph)
{
	...
	{
		this.status = value;
		this.fireEvent(new mxg.mxEventObject('statusChanged'));
	};
...
{
	this.autosave = value;
	this.fireEvent(new mxg.mxEventObject('autosaveChanged'));
};
...
Editor.prototype.readGraphState = function(node)
{
	...
	if (pw != null && ph != null)
	{
		this.graph.pageFormat = new mxg.mxRectangle(0, 0, parseFloat(pw), parseFloat(ph));
	}
...
Editor.prototype.setGraphXml = function(node)
{
	if (node != null)
	{
		...
		if (node.nodeName == 'mxGraphModel')
		{
			...
			this.fireEvent(new mxg.mxEventObject('resetGraphView'));
		}
...
Editor.prototype.getGraphXml = function(ignoreSelection)
{
	...
	if (ignoreSelection)
	{
		var enc = new mxg.mxCodec(mxUtils.createXmlDocument());
...
Editor.prototype.updateGraphComponents = function()
{
	...
	if (graph.container != null)
	{
		...
		this.fireEvent(new mxg.mxEventObject('updateGraphComponents'));
	}
};
...
{
	var graph = this.graph;
	var undoMgr = new mxg.mxUndoManager();
...
(function()
{
				...
				if (this.backgroundPageShape == null)
				{
					...
					if (firstChild != null)
					{
						...
						mxEvent.addGestureListeners(this.backgroundPageShape.node,
							mxUtils.bind(this, function(evt)
							{
								graph.fireMouseEvent(mxEvent.MOUSE_DOWN, new mxg.mxMouseEvent(evt));
							}),
							mxUtils.bind(this, function(evt)
							{
								...
								{
									graph.fireMouseEvent(mxEvent.MOUSE_MOVE, new mxg.mxMouseEvent(evt));
								}
							}),
							mxUtils.bind(this, function(evt)
							{
								graph.fireMouseEvent(mxEvent.MOUSE_UP, new mxg.mxMouseEvent(evt));
							})
						);
					}
				}
	...
	mxGraphPanGraph = mxGraph.prototype.panGraph;
	...
	mxGraph.prototype.updatePageBreaks = function(visible, width, height)
	{
		...
		var bounds = new mxg.mxRectangle(scale * tr.x, scale * tr.y, fmt.width * ps, fmt.height * ps);
			...
			if (breaks != null)
			{
				...
				for (var i = 0; i <= count; i++)
				{
					var pts = (breaks == this.horizontalPageBreaks) ?
						[new mxg.mxPoint(Math.round(bounds2.x), Math.round(bounds2.y + (i + 1) * bounds.height)),
						 new mxg.mxPoint(Math.round(right), Math.round(bounds2.y + (i + 1) * bounds.height))] :
						[new mxg.mxPoint(Math.round(bounds2.x + (i + 1) * bounds.width), Math.round(bounds2.y)),
						 new mxg.mxPoint(Math.round(bounds2.x + (i + 1) * bounds.width), Math.round(bottom))];
					...
					{
						var pageBreak = new mxg.mxPolyline(pts, this.pageBreakColor);
	...
	{
		return new mxg.mxRectangleShape(bounds, '#ffffff', '#cacaca');
	};
	...
	mxGraphView.prototype.getBackgroundPageBounds = function()
	{
		...
		var bounds = new mxg.mxRectangle(this.scale * (this.translate.x + x0 * pw), this.scale *
	...
	graphHandlerIsDelayedSelection = mxGraphHandler.prototype.isDelayedSelection;
