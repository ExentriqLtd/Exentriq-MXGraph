			...
			if (evt.getProperty('eventName') == 'mouseDown' && this.isEnabled())
			{
				...
				if (!mxEvent.isControlDown(me.getEvent()) && !mxEvent.isShiftDown(me.getEvent()))
		    	{
			    	...
			    	if (state != null)
			    	{
			    		...
			    		{
			    			start.point = new mxg.mxPoint(me.getGraphX(), me.getGraphY());
				    					...
				    					handle == handler.bends.length - 1 || state.visibleTargetState != null)
				    				{
				    					if (!entity && handle != mxEvent.LABEL_HANDLE)
				    					{
					    						...
					    						edgeStyle == mxEdgeStyle.OrthConnector))
					    					{
					    						...
					    						if (handle == null)
					    						{
							    					var box = new mxg.mxRectangle(start.point.x, start.point.y);
			    	...
			    	else
			    	{
				    	...
				    	if (state != null)
				    	{
				    		...
				    		if (this.model.isEdge(state.cell))
				    		{
			    				...
			    				if (pts != null)
			    				{
			    					var box = new mxg.mxRectangle(me.getGraphX(), me.getGraphY());
			    					box.grow(mxEdgeHandler.prototype.handleImage.width / 2);
	...
	if (typeof mxVertexHandler !== 'undefined')
	{
		...
		this.graphHandler.getGuideStates = function()
		{
			...
			if (this.graph.pageVisible)
			{
				...
				{
					guides.push(new mxg.mxRectangle(((layout.x + i) * pw + t.x) * s,
						(layout.y * ph + t.y) * s, pw * s, ph * s));
				}

				for (var j = 0; j < layout.height; j++)
				{
					guides.push(new mxg.mxRectangle((layout.x * pw + t.x) * s,
	    ...
	    var rubberband = new mxg.mxRubberband(this);
 ...
 */
Graph.prototype.scrollTileSize = new mxg.mxRectangle(0, 0, 400, 400);
...
{
	this.layoutManager = new mxg.mxLayoutManager(this);
	...
	this.layoutManager.getLayout = function(cell)
	{
		...
		{
			var stackLayout = new mxg.mxStackLayout(this.graph, true);
		...
		{
			var treeLayout = new mxg.mxCompactTreeLayout(this.graph);
		...
		{
			var flowLayout = new mxg.mxHierarchicalLayout(this.graph, mxUtils.getValue(style,
...
{
	return (this.pageVisible) ? new mxg.mxRectangle(0, 0, this.pageFormat.width * this.pageScale,
...
Graph.prototype.getPageLayout = function()
{
	...
	{
		return new mxg.mxRectangle(0, 0, 1, 1);
	}
	else
	{
		...
		return new mxg.mxRectangle(x0, y0, w0, h0);
	}
};
...
{
	this.gridSize = value;
	this.fireEvent(new mxg.mxEventObject('gridSizeChanged'));
};
...
Graph.prototype.connectVertex = function(source, direction, length, evt, forceClone)
{
	var pt = (source.geometry.relative && source.parent.geometry != null) ?
			new mxg.mxPoint(source.parent.geometry.width * source.geometry.x, source.parent.geometry.height * source.geometry.y) :
			new mxg.mxPoint(source.geometry.x, source.geometry.y);
...
//			edge.style = mxUtils.setStyle(edge.style, 'elbow', elbowValue);
		...
		if (edge != null)
		{
			this.fireEvent(new mxg.mxEventObject('cellsInserted', 'cells', [edge]));
		}
	}
	finally
	{
...
HoverIcons.prototype.computeBoundingBox = function()
{
	...
	this.visitNodes(function(elt)
	{
		if (elt.parentNode != null)
		{
			var tmp = new mxg.mxRectangle(elt.offsetLeft, elt.offsetTop, elt.offsetWidth, elt.offsetHeight);
...
(function()
{
	...
	mxGraphView.prototype.updateFloatingTerminalPoint = function(edge, start, end, source)
	{
			...
			edge.style['snapToPoint'] == '1'))
		{
		    ...
		    var center = new mxg.mxPoint(start.getCenterX(), start.getCenterY());
		...
		if (state.style != null && typeof(pako) !== 'undefined')
		{
	    	...
	    	if (shape != null && shape.substring(0, 8) == 'stencil(')
	    	{
	    		try
	    		{
	    			...
	    			return new mxg.mxShape(new mxg.mxStencil(doc.documentElement));
	    		}
	    		catch (e)
	    		{
		...
		while (shape != null)
		{
			if (shape.nodeType == mxConstants.NODETYPE_ELEMENT)
			{
				...
				if (name != null)
				{
					...
					if (install)
					{
						mxStencilRegistry.addStencil(packageName + stencilName.toLowerCase(), new mxg.mxStencil(shape));
					}
...
if (typeof mxVertexHandler != 'undefined')
{
	(function()
	{
		...
		mxConstants.HANDLE_FILLCOLOR = '#FFFFFF';//'#99ccff';
		...
		{
			var hl = new mxg.mxEllipse(null, this.highlightColor, this.highlightColor, 0);
		...
		mxConnectionHandler.prototype.createEdgeState = function(me)
		{
			...
			var state = new mxg.mxCellState(this.graph.view, edge, this.graph.getCellStyle(edge));
		...
		Graph.prototype.loadStylesheet = function()
		{
			...
			if (node != null)
			{
				var dec = new mxg.mxCodec(node.ownerDocument);
			...
			if (terminal != null)
			{
				...
				if (constraints != null)
				{
					...
					try
					{
						...
						{
							var tmp = c[i];
							result.push(new mxg.mxConnectionConstraint(new mxg.mxPoint(tmp[0], tmp[1]), (tmp.length > 2) ? tmp[2] != '0' : true));
						}
					}
					catch (e)
					{
...
/*			if (change instanceof mxValueChange && change.cell.value != null &&
		...
		Graph.prototype.getInsertPoint = function()
		{
			...
			return new mxg.mxPoint(this.snap(dx + gs), this.snap(dy + gs));
		};
		...
		Graph.prototype.addText = function(x, y, state)
		{
			...
			var label = new mxg.mxCell();
			label.value = 'Text';
			label.style = 'text;html=1;resizable=0;points=[];'
			label.geometry = new mxg.mxGeometry(0, 0, 0, 0);
			label.vertex = true;
			...
			if (state != null)
			{
				...
				label.geometry.offset = new mxg.mxPoint(0, 0);
				pt2 = this.view.getPoint(state, label.geometry);

				var scale = this.view.scale;
				label.geometry.offset = new mxg.mxPoint(Math.round((x - pt2.x) / scale), Math.round((y - pt2.y) / scale));
			}
			else
			{
			...
			try
			{
				...
				this.fireEvent(new mxg.mxEventObject('textInserted', 'cells', [label]));
		...
		Graph.prototype.addClickHandler = function(highlight, beforeClick, onClick)
		{
			...
			var mouseListener =
			{
			    ...
			    highlight: (highlight != null && highlight != '' && highlight != mxConstants.NONE) ?
			    	new mxg.mxCellHighlight(graph, highlight, 4) : null,
			    startX: 0,
			    startY: 0,
			    scrollLeft: 0,
			    scrollTop: 0,
		...
		Graph.prototype.getSvg = function(background, scale, border, nocrop, crisp, ignoreSelection, showText)
		{
			...
			if (bgImg != null)
			{
				var s2 = vs / scale;
				var tr = this.view.translate;
				var tmp = new mxg.mxRectangle(tr.x * s2, tr.y * s2, bgImg.width * s2, bgImg.height * s2);
		...
		{
			return new mxg.mxSvgCanvas2D(node);
		};
		...
		{
			var guide = new mxg.mxPolyline([], mxConstants.GUIDE_COLOR, mxConstants.GUIDE_STROKEWIDTH);
		...
		mxCellEditor.prototype.getMinimumSize = function(state)
		{
			...
			return new mxg.mxRectangle(0, 0, (state.text == null) ? 30 :  state.text.size * scale + 20, 30);
		};
		...
		{
			var result = new mxg.mxPoint(0, 0);
			var tol = this.tolerance;
		...
		mxRubberband.prototype.mouseUp = function(sender, me)
		{
			...
			if (execute)
			{
				...
				{
					var rect = new mxg.mxRectangle(this.x, this.y, this.width, this.height);
		...
		mxEdgeHandler.prototype.createHandleShape = function(index, virtual)
		{
			...
			if (img != null)
			{
				var shape = new mxg.mxImageShape(new mxg.mxRectangle(0, 0, img.width, img.height), img.src);
			...
			else
			{
				...
				return new mxg.mxRectangleShape(new mxg.mxRectangle(0, 0, s, s), mxConstants.HANDLE_FILLCOLOR, mxConstants.HANDLE_STROKECOLOR);
			}
		};
		...
		mxVertexHandler.prototype.getSelectionBounds = function(state)
		{
			...
			if (model.isEdge(parent) && geo != null && geo.relative && state.width < 2 && state.height < 2 && state.text != null && state.text.boundingBox != null)
			{
				...
				return new mxg.mxRectangle(Math.round(bbox.x), Math.round(bbox.y), Math.round(bbox.width), Math.round(bbox.height));
			}
			else
			{
		...
		mxVertexHandler.prototype.redrawHandles = function()
		{
			...
			{
				var c = new mxg.mxPoint(this.state.getCenterX(), this.state.getCenterY());
				var tmp = new mxg.mxRectangle(this.state.x, this.state.y - 22, this.state.width + 24, this.state.height + 22);
		...
		mxEdgeHandler.prototype.redrawHandles = function()
		{
			...
			if (this.marker != null)
			{
				...
				if (this.state != null && this.linkHint != null)
				{
					...
					{
						b = new mxg.mxRectangle(b.x, b.y, b.width, b.height);
