 ...
 */
Dialog = function(editorUi, elt, w, h, modal, closable, onClose)
{
	var dx = 0;
	...
	editorUi.editor.fireEvent(new mxg.mxEventObject('showDialog'));
};
 ...
 */
ColorDialog = function(editorUi, color, apply, cancelFn)
{
 ...
 */
PageSetupDialog = function(editorUi)
{
...
//	var changeImageLink = document.createElement('input');
//	changeImageLink.type = 'file';
...
/*	mxEvent.addListener(changeImageLink, 'change', function(evt) {
...
PageSetupDialog.addPageFormatPanel = function(div, namePostfix, pageFormat, pageFormatListener)
{
	...
	var update = function()
	{
		...
		var newPageFormat = new mxg.mxRectangle(0, 0, parseInt(widthInput.value), parseInt(heightInput.value));

		if (paperSizeSelect.value != 'custom' && landscapeCheckBox.checked)
		{
			newPageFormat = new mxg.mxRectangle(0, 0, newPageFormat.height, newPageFormat.width);
		}
...
PageSetupDialog.getFormats = function()
{
	return [{key: 'letter', title: 'US-Letter (8,5" x 11")', format: mxConstants.PAGE_FORMAT_LETTER_PORTRAIT},
	        {key: 'legal', title: 'US-Legal (8,5" x 14")', format: new mxg.mxRectangle(0, 0, 850, 1400)},
	        {key: 'tabloid', title: 'US-Tabloid (279 mm x 432 mm)', format: new mxg.mxRectangle(0, 0, 1100, 1700)},
	        {key: 'a3', title: 'A3 (297 mm x 420 mm)', format: new mxg.mxRectangle(0, 0, 1169, 1652)},
	        {key: 'a4', title: 'A4 (210 mm x 297 mm)', format: mxConstants.PAGE_FORMAT_A4_PORTRAIT},
	        {key: 'a5', title: 'A5 (148 mm x 210 mm)', format: new mxg.mxRectangle(0, 0, 584, 826)},
 ...
 */
PrintDialog = function(editorUi, title)
{
...
{
	var preview = new mxg.mxPrintPreview(graph, scale, pf, border, x0, y0);
 ...
 */
FilenameDialog = function(editorUi, filename, buttonText, fn, label, validateFn, content, helpLink, closeOnBtn, cancelFn)
{
 ...
 */
TextareaDialog = function(editorUi, title, url, fn, cancelFn, cancelTitle, w, h, addButtons, noHide, noWrap, applyTitle)
{
	w = (w != null) ? w : 300;
	h = (h != null) ? h : 120;
 ...
 */
EditDiagramDialog = function(editorUi)
{
	var div = document.createElement('div');
 ...
 */
ExportDialog = function(editorUi)
{
	var graph = editorUi.editor.graph;
...
ExportDialog.exportFile = function(editorUi, name, format, bg, s, b)
{
    ...
    // Renders graph. Offset will be multiplied with state's scale when painting state.
		var xmlCanvas = new mxg.mxXmlCanvas2D(root);
		...
		var imgExport = new mxg.mxImageExport();
    imgExport.drawState(graph.getView().getState(graph.model.root), xmlCanvas);
		...
		if (param.length <= MAX_REQUEST_SIZE && w * h < MAX_AREA)
			{
				editorUi.hideDialog();
				var req = new mxg.mxXmlRequest(EXPORT_URL, 'format=' + format +
...
ExportDialog.saveLocalFile = function(editorUi, data, filename, format)
{
	...
	{
		editorUi.hideDialog();
		var req = new mxg.mxXmlRequest(SAVE_URL, 'xml=' + encodeURIComponent(data) + '&filename=' +
...
EditDataDialog = function(ui, cell)
{
	...
	var form = new mxg.mxForm('properties');
 ...
 */
LinkDialog = function(editorUi, initialValue, btnLabel, fn)
{
	var div = document.createElement('div');
 ...
 *
 */
OutlineWindow = function(editorUi, x, y, w, h)
{
	var graph = editorUi.editor.graph;
	...
	this.window = new mxg.mxWindow(mxResources.get('outline'), div, x, y, w, h, true, true);
 ...
 *
 */
LayersWindow = function(editorUi, x, y, w, h)
{
	var graph = editorUi.editor.graph;
	...
	mxEvent.addListener(removeLink, 'click', function(evt)
	{
		if (graph.isEnabled())
		{
			...
			try
			{
				...
				{
					graph.model.add(graph.model.root, new mxg.mxCell());
	...
	mxEvent.addListener(addLink, 'click', function(evt)
	{
		if (graph.isEnabled())
		{
			...
			try
			{
				var cell = graph.addCell(new mxg.mxCell(mxResources.get('untitledLayer')), graph.model.root);
	...
	this.window = new mxg.mxWindow(mxResources.get('layers'), div, x, y, w, h, true, true);
	this.window.destroyOnClose = false;
	this.window.setMaximizable(false);
	this.window.setResizable(true);
	this.window.setClosable(true);
