...
Menus.prototype.init = function()
{
	...
	this.put('layout', new Menu(mxUtils.bind(this, function(menu, parent)
	{
		...
		{
			var layout = new mxg.mxHierarchicalLayout(graph, mxConstants.DIRECTION_WEST);
		...
		{
			var layout = new mxg.mxHierarchicalLayout(graph, mxConstants.DIRECTION_NORTH);
		...
		menu.addItem(mxResources.get('horizontalTree'), null, mxUtils.bind(this, function()
		{
			...
			if (tmp != null)
			{
				var layout = new mxg.mxCompactTreeLayout(graph, true);
		...
		menu.addItem(mxResources.get('verticalTree'), null, mxUtils.bind(this, function()
		{
			...
			if (tmp != null)
			{
				var layout = new mxg.mxCompactTreeLayout(graph, false);
				layout.edgeRouting = false;
				layout.levelDistance = 30;
		...
		menu.addItem(mxResources.get('radialTree'), null, mxUtils.bind(this, function()
		{
			...
			if (tmp != null)
			{
				var layout = new mxg.mxRadialTreeLayout(graph, false);
		...
		{
			var layout = new mxg.mxFastOrganicLayout(graph);
		...
		{
			var layout = new mxg.mxCircleLayout(graph);
	...
	{
		this.addMenuItems(menu, ['new', 'open', '-', 'save', 'saveAs', '-', 'import', 'export', '-', 'pageSetup', 'print','-','exit'], parent);
	})));
...
Menus.prototype.edgeStyleChange = function(menu, label, keys, values, sprite, parent, reset)
{
	return menu.addItem(label, null, mxUtils.bind(this, function()
	{
		...
		try
		{
			...
			this.editorUi.fireEvent(new mxg.mxEventObject('styleChanged', 'keys', keys,
...
Menus.prototype.createStyleChangeFunction = function(keys, values)
{
	return mxUtils.bind(this, function()
	{
		...
		try
		{
			...
			this.editorUi.fireEvent(new mxg.mxEventObject('styleChanged', 'keys', keys, 'values', values,
...
Menus.prototype.toggleStyle = function(key, defaultValue)
{
	...
	this.editorUi.fireEvent(new mxg.mxEventObject('styleChanged', 'keys', [key], 'values', [value],
			'cells', graph.getSelectionCells()));
};
	...
	if (graph.getSelectionCount() > 0)
	{
		...
		if (state != null)
		{
				...
				mxUtils.getValue(state.style, mxConstants.STYLE_SHAPE, null) != 'arrow')
			{
				...
				{
					var index = handler.getHandleForEvent(graph.updateMouseEvent(new mxg.mxMouseEvent(evt)));
	...
	if (funct != null)
	{
		...
		var clickHandler = mxUtils.bind(this, function(evt)
		{
			if (show && elt.enabled == null || elt.enabled)
			{
				...
				var menu = new mxg.mxPopupMenu(funct);
