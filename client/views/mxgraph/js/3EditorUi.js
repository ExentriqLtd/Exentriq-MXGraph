  ...
  // Escape key hides dialogs, adds space+drag panning
	...
	this.setDefaultStyle = function(cell)
	{
		...
		if (state != null)
		{
			...
			this.fireEvent(new mxg.mxEventObject('styleChanged', 'keys', keys, 'values', values, 'cells', [state.cell]));
		}
	};
	...
	this.clearDefaultStyle = function()
	{
		...
		// Updates UI
		this.fireEvent(new mxg.mxEventObject('styleChanged', 'keys', [], 'values', [], 'cells', []));
	};
...
EditorUi.prototype.initCanvas = function()
{
	...
	{
		return new mxg.mxPoint(Math.max(0, Math.round((graph.container.offsetWidth - 34) / graph.view.scale)),
	...
	graph.view.getBackgroundPageBounds = function()
	{
		...
		return new mxg.mxRectangle(this.scale * (this.translate.x + layout.x * page.width),
	...
	graph.getPreferredPageSize = function(bounds, width, height)
	{
		...
		return new mxg.mxRectangle(0, 0, pages.width * size.width, pages.height * size.height);
	};
	...
	else if (this.editor.extendCanvas)
	{
		...
		graph.sizeDidChange = function()
		{
			if (this.container != null && mxUtils.hasScrollbars(this.container))
			{
				...
				{
					graph.minimumGraphSize = new mxg.mxRectangle(0, 0, minw, minh);
				}
	...
	mxEvent.addMouseWheelListener(mxUtils.bind(this, function(evt, up)
	{
			...
			graph.panningHandler.isActive()) && (this.dialogs == null || this.dialogs.length == 0))
		{
			...
			while (source != null)
			{
				...
				{
					cursorPosition = new mxg.mxPoint(mxEvent.getClientX(evt), mxEvent.getClientY(evt));
					graph.lazyZoom(up);
					mxEvent.consume(evt);
...
EditorUi.prototype.toggleFormatPanel = function(forceHide)
{
	...
	this.refresh();
	this.format.refresh();
	this.fireEvent(new mxg.mxEventObject('formatWidthChanged'));
};
...
EditorUi.prototype.open = function()
{
	...
	this.editor.fireEvent(new mxg.mxEventObject('resetGraphView'));
};
...
EditorUi.prototype.setScrollbars = function(value)
{
	...
	this.fireEvent(new mxg.mxEventObject('scrollbarsChanged'));
};
...
EditorUi.prototype.setPageVisible = function(value)
{
	...
	this.fireEvent(new mxg.mxEventObject('pageViewChanged'));
};
...
EditorUi.prototype.setBackgroundColor = function(value)
{
	...
	this.fireEvent(new mxg.mxEventObject('backgroundColorChanged'));
};
...
EditorUi.prototype.setFoldingEnabled = function(value)
{
	...
	this.fireEvent(new mxg.mxEventObject('foldingEnabledChanged'));
};
...
EditorUi.prototype.setPageFormat = function(value)
{
	...
	this.fireEvent(new mxg.mxEventObject('pageFormatChanged'));
};
...
EditorUi.prototype.setPageScale = function(value)
{
	...
	this.fireEvent(new mxg.mxEventObject('pageScaleChanged'));
};
...
EditorUi.prototype.setGridColor = function(value)
{
	...
	this.editor.graph.view.validateBackground();
	this.fireEvent(new mxg.mxEventObject('gridColorChanged'));
};
...
EditorUi.prototype.addSplitHandler = function(elt, horizontal, dx, onChange)
{
	...
	function moveHandler(evt)
	{
		if (start != null)
		{
			var pt = new mxg.mxPoint(mxEvent.getClientX(evt), mxEvent.getClientY(evt));
	...
	{
		start = new mxg.mxPoint(mxEvent.getClientX(evt), mxEvent.getClientY(evt));
		initial = getValue();
		ignoreClick = false;
		mxEvent.consume(evt);
	});
...
EditorUi.prototype.hideDialog = function(cancel)
{
	if (this.dialogs != null && this.dialogs.length > 0)
	{
		...
		this.editor.fireEvent(new mxg.mxEventObject('hideDialog'));
	}
};
	...
	if (graph.isEnabled())
	{
		...
		finally
		{
			...
			if (this.allowAnimation && animate && navigator.userAgent.indexOf('Camino') < 0)
			{
				...
				var morph = new mxg.mxMorphing(graph);
...
EditorUi.prototype.showBackgroundImageDialog = function(apply)
{
	...
	if (newValue != null && newValue.length > 0) {
		var img = new Image();
		img.onload = function()	{
			apply(new mxg.mxImage(newValue, img.width, img.height));
		};
		img.onerror = function() {
			apply(null);
			mxUtils.alert(mxResources.get('fileNotFound'));
...
EditorUi.prototype.setBackgroundImage = function(image)
{
	...
	this.fireEvent(new mxg.mxEventObject('backgroundImageChanged'));
};
...
{
	var outline = new mxg.mxOutline(this.editor.graph);
	outline.border = 20;
...
EditorUi.prototype.createKeyHandler = function(editor)
{
	var editorUi = this;
	var graph = this.editor.graph;
	var keyHandler = new mxg.mxKeyHandler(graph);
